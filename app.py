...
# CSS Ajustado para transformar containers nativos em Cards Premium
st.markdown(f"""
    <style>
        ...
        /* ESTILO DOS CARDS (Aplicado aos containers nativos com border=True) */
...
if not df_base.empty:
    ...
    with aba1:
        ...
        if not df_filtrado.empty:
            ...
            with col_esq:
                with st.container(border=True): # Card Nativo
            ...
            with col_dir:
                with st.container(border=True): # Card Nativo
                    st.markdown(f'<div style="color:{COR_SECUNDARIA}; font-weight:bold;">VOLUME DE RODAGEM MENSAL</div>', unsafe_allow_html=True)
    ...
    with aba2:
        ...
        # --- CORREÇÃO FINAL: Título e Gráfico dentro do Container Nativo ---
        with st.container(border=True):
        ...
        # Alertas de performance (3 níveis)
        if desvio > 10:
            st.markdown(f'<div class="alert-box alert-error">🚨 <b>ALERTA CRÍTICO:</b> {mes_diag} superou a meta em {desvio:.1f}%. Recomenda-se auditoria.</div>', unsafe_allow_html=True)
        elif desvio > 0:
            st.markdown(f'<div class="alert-box alert-warning">⚠️ <b>ATENÇÃO:</b> {mes_diag} operou {desvio:.1f}% ACIMA da meta.</div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div class="alert-box alert-success">✅ <b>DENTRO DA META:</b> {mes_diag} operou com economia de {abs(desvio):.1f}%.</div>', unsafe_allow_html=True)
else:
