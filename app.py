...
alt.themes.enable("default") # Garante fundo branco nos gráficos

# --- 1. IDENTIDADE VISUAL & CORES ---
COR_PRIMARIA = "#10b981"   # Verde
COR_SECUNDARIA = "#334155" # Slate Blue
...
# CSS Avançado: Unificando estilo, visibilidade e alertas
st.markdown(f"""
    <style>
        ...
        /* Classes de Alertas (Texto em PRETO conforme solicitado) */
...
def carregar_dados():
    try:
        ...
        # Mapeamento para nomes completos
...
with st.sidebar:
    ...
    if not df_base.empty:
        ...
        filtro_gasto = st.slider("Faixa de Investimento (R$):", g_min, g_max, (g_min, g_max))
        filtro_ef = st.slider("Custo Máximo por KM (R$):", 0.0, float(df_base['Custo/KM'].max()), float(df_base['Custo/KM'].max()))
    st.caption("Gabriel Barbosa | Analista")

# --- 4. INTERFACE ---
if not df_base.empty:
    df_filtrado = df_base[(df_base['Mês'].isin(meses_sel)) & (df_base['Gasto'].between(filtro_gasto[0], filtro_gasto[1])) & (df_base['Custo/KM'] <= filtro_ef)]
    ...
    with aba1:
        ...
        if not df_filtrado.empty:
            ...
            with col_esq:
                st.markdown('<div class="chart-container"><b>EVOLUÇÃO DOS CUSTOS TOTAIS</b>', unsafe_allow_html=True)
            ...
            with col_dir:
                st.markdown('<div class="chart-container"><b>VOLUME DE RODAGEM MENSAL</b>', unsafe_allow_html=True)
            ...
            st.markdown('<div class="chart-container"><b>DETALHAMENTO TÉCNICO DOS REGISTROS</b>', unsafe_allow_html=True)
    ...
    with aba2:
        ...
        # --- LÓGICA DE ALERTA DE 3 NÍVEIS (CORREÇÃO AGOSTO/DEZEMBRO) ---
        if desvio > 10:
            ...
            st.markdown(f'<div class="alert-box alert-warning">⚠️ <b>ATENÇÃO:</b> {mes_diag} operou {desvio:.1f}% ACIMA da meta. Custo saiu da risca planejada.</div>', unsafe_allow_html=True)
        else:
